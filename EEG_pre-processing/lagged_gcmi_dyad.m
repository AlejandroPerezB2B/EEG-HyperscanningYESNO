    ...
    if numberOfSurrogates > 0
        ...
        if options.UseParallel
            ...
            if license('test', 'Distrib_Computing_Toolbox') ~= 1
                error(['UseParallel=true requires the MATLAB Parallel ' ...
                    'Computing Toolbox.']);
            end
            ...
            if isempty(pool)
                try
                    fprintf('Starting a parallel pool...\n');
                    pool = parpool;
                catch ME
                    error('HyperYESNO:ParallelPoolStartFailed', ...
                        ['UseParallel=true, but MATLAB could not start a ' ...
                        'parallel pool:\n%s'], ...
                        ME.message);
                end
            end
