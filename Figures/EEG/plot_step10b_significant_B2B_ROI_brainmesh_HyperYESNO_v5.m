function [plotData, summaryStruct] = plot_step10b_significant_B2B_ROI_brainmesh_HyperYESNO_v5(step10bMatFile, varargin)
...
%     'SaveFigures', true);
% 
% The command used for the paper:
% [plotData, summaryStruct] = ...
%     plot_step10b_significant_B2B_ROI_brainmesh_HyperYESNO_v5( ...
%     'E:\EEG_data_HyperYESNO\Group_GCMI\Step10b_HyperYESNO_spatial_cluster_permutation.mat', ...
%     'FsAverageDir', 'E:\EEG_data_HyperYESNO\fsaverage', ...
%     'LagsToPlot', [-250 300], ...
%     'ShowROILabels', false, ...
%     'ArcHeightMultiplier', 2, ...
%     'LinkWidthRange', [0.8 2.0], ...
%     'PanelPaddingFraction', 0.1, ...
%     'SaveFigures', true);
% 
% -------------------------------------------------------------------------
% USEFUL OPTIONS
...
addParameter(p, 'ShowROILabels', false, @(x)islogical(x) && isscalar(x));
...
addParameter(p, 'PanelPaddingFraction', 0.10, @(x)isnumeric(x) && isscalar(x) && x>=0);
addParameter(p, 'MinimumPanelPadding', 8.0, @(x)isnumeric(x) && isscalar(x) && x>=0);
...
baseArcHeight = max(55, 0.24*brainSeparation);
if isempty(o.ArcHeight)
    % ArcHeightMultiplier now directly controls the vertical curvature.
    % 1.0 = original automatic arc; 3.0 = three times more pronounced.
    arcHeight = o.ArcHeightMultiplier * baseArcHeight;
else
    % Explicit ArcHeight overrides the multiplier.
...
for k = 1:nPanel
    ...
    % Record the actual rendered 3-D bounds. This is more robust than
    % calculating limits from the unrotated template dimensions and prevents
    % the occipital poles from being clipped after the two brains are yawed
    % to face each other. Common padded limits are applied after both panels
    % have been constructed.
    panelBounds(k,:) = collect_panel_bounds(ax);
...
% Apply identical data-driven limits to both panels. The bounds are based on
% the transformed pial meshes, ROI nodes, curved connections, and role labels.
validBounds = all(isfinite(panelBounds),2);
if any(validBounds)
    commonBounds = [ ...
        min(panelBounds(validBounds,1)), max(panelBounds(validBounds,2)), ...
        min(panelBounds(validBounds,3)), max(panelBounds(validBounds,4)), ...
        min(panelBounds(validBounds,5)), max(panelBounds(validBounds,6))];
    commonBounds = pad_panel_bounds(commonBounds, o.PanelPaddingFraction, o.MinimumPanelPadding);
    for k = 1:nPanel
        xlim(panelAxes(k), commonBounds(1:2));
        ylim(panelAxes(k), commonBounds(3:4));
        zlim(panelAxes(k), commonBounds(5:6));
    end
end
...
summaryStruct.baseArcHeight = baseArcHeight;
summaryStruct.arcHeightMultiplier = o.ArcHeightMultiplier;
summaryStruct.panelPaddingFraction = o.PanelPaddingFraction;
...
text(ax,mean(allK(:,1)),mean(allK(:,2)),max(allK(:,3))+o.RoleLabelVerticalOffset,'KNOWER', ...
    ...
    'BackgroundColor','w','Margin',2,'Interpreter','none','Tag','RoleLabel');
...
%% ========================================================================
function b = collect_panel_bounds(ax)
% Return [xmin xmax ymin ymax zmin zmax] from the actual plotted 3-D data.
% Text objects are ignored except for the KNOWER/GUESSER role labels, which
% are tagged as 'RoleLabel'.

x = []; y = []; z = [];
objs = allchild(ax);
for ii = 1:numel(objs)
    h = objs(ii);
    typ = lower(string(get(h,'Type')));
    try
        switch typ
            case "patch"
                v = get(h,'Vertices');
            ...
            case {"line","scatter","surface"}
                xd = get(h,'XData'); yd = get(h,'YData'); zd = get(h,'ZData');
                if isnumeric(xd) && isnumeric(yd) && isnumeric(zd)
                    x = [x; xd(:)]; %#ok<AGROW>
                    y = [y; yd(:)]; %#ok<AGROW>
                    z = [z; zd(:)]; %#ok<AGROW>
                end
            case "text"
                if strcmp(string(get(h,'Tag')),"RoleLabel")
                    p = get(h,'Position');
        ...
        % Ignore graphics objects without usable 3-D data.
    end
end
...
if isempty(x)
    ...
    b = [min(x) max(x) min(y) max(y) min(z) max(z)];
end
end


%% ========================================================================
function b = pad_panel_bounds(b, frac, minPad)
% Add symmetric padding to data-driven panel bounds.
rx = max(b(2)-b(1), eps);
ry = max(b(4)-b(3), eps);
rz = max(b(6)-b(5), eps);
px = max(minPad, frac*rx);
py = max(minPad, frac*ry);
pz = max(minPad, frac*rz);
b = [b(1)-px b(2)+px b(3)-py b(4)+py b(5)-pz b(6)+pz];
end
